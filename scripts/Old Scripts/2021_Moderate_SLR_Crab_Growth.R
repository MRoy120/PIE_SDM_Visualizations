...
moderate_SLR_crab_growth <- read_csv("data/scenario_6_crab_growth/2021_Moderate_SLR_Crab_Growth.csv") %>%
...
ggplot(data = moderate_SLR_crab_growth_2, 
       mapping = aes(x = time,
                     y = areal_extent,
                     color = `Habitat Type`)) +
  geom_line(size = 6,
  ...
  ggsave(filename = "figures/moderate_SLR_crab_growth.png",
         width = 22,
         height = 15)

...
moderate_SLR_flood_mit <- read_csv("data/2021_Moderate_SLR_Flood_Mit.csv") %>%
...
moderate_SLR_flood_mit_2 <- moderate_SLR_flood_mit %>%
  pivot_longer(cols = `Total Marsh`:`Low Marsh`,
               names_to = "Habitat Type",
               values_to = "areal_extent")

ggplot(data = moderate_SLR_flood_mit_2, mapping = aes(x = time,
                                             y = areal_extent,
                                             color = `Habitat Type`)) +
  geom_line(size = 6,
  ...
  labs(title = "Plum Island Estuary Salt Marsh Extent",
       subtitle = "Marsh Area by Habitat Type at\nModerate Sea Level Rise Without Crabs, with Tidal Flood Mitigation") +
  theme(plot.title = element_text(size = 42, 
                                  face = "bold"),
        plot.subtitle = element_text(size = 38,
                                     face = "bold"),
        legend.position = "bottom") +
  ggsave(filename = "scripts/simulation_visualization/figures/moderate_SLR_flood_mit.png",
         width = 22,
         height = 15)

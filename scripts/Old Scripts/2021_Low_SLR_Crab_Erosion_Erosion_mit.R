...
low_SLR_erosion_mit <- read_csv("data/2021_Crab_Erosion_Low_SLR_Erosion_Mit.csv") %>%
...
low_SLR_erosion_mit_2 <- low_SLR_erosion_mit %>%
  pivot_longer(cols = `Total Marsh`:`Low Marsh`,
               names_to = "Habitat Type",
               values_to = "areal_extent")

ggplot(data = low_SLR_erosion_mit_2, mapping = aes(x = time,
                                             y = areal_extent,
                                             color = `Habitat Type`)) +
  geom_line(size = 6,
  ...
  labs(title = "Plum Island Estuary Salt Marsh Extent",
       subtitle = "Marsh Area by Habitat Type at\nLow Sea Level Rise With Crabs, with Erosion Reduction") +

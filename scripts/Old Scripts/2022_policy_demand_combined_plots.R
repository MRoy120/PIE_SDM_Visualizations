...
###
# Combining SLR Scenario Data for Erosion Control Demand
total_slr_demand <- rbind(low_crabs_join_2,
                          mid_crabs_join_2,
...
total_crabs_erosion_demand_plot <- ggplot(data = total_slr_demand, 
                                          mapping = aes(x = time,
  ...
  labs(x = "Year",
       ...
       title = "Marsh Erosion by Fiddler Crabs"
  ) +
  standard_theme +
  theme(legend.position = "none") +
  theme_bw(base_size = 40,
           base_family = "Arial") +
  ylab(expression(Policy~Demand~(percent~year^{"-1"}))) +
  theme(axis.title.y = element_text(size = 42, 
                                    face = "bold")) +
  theme(legend.key.height = unit(3, 'cm'))

total_crabs_erosion_demand_plot

ggsave(filename = "figures/policy_demand.png",
       width = 30,
       height = 15)

###
# Combining SLR Scenario Data for Erosion
total_crabs_flood_demand_plot <- ggplot(data = total_slr_demand, 
                                        mapping = aes(x = time,
  ...
  labs(x = "Year",
       #y = "Average Time Recreating\n(days/year)",
       #title = "High Sea Level Rise Rate and\nMarsh Erosion by Fiddler Crabs with Mitigation Policies"
       title = "Policy Demand: Tidal Flood Control"
  ) +
  standard_theme +
  theme(legend.position = "none") +
  theme_bw(base_size = 40,
...
total_crabs_flood_demand_plot

ggsave(filename = "figures/total_slr_growth_rec.png",
       width = 30,
       height = 15)

ggpubr::ggarrange(plotlist = (list(total_crabs_erosion_demand_plot,
                                   total_crabs_flood_demand_plot)),
                  nrow = 2,
                  ncol = 1,
                  legend = "right")

ggsave(filename = "figures/policy_demand_plots.png",
       width = 35,
       height = 30)

###
# Combining SLR Scenario Data for Erosion
total_slr_demand_growth <- rbind(low_crab_growth_join_2,
...
total_crabs_growth_demand_plot <- ggplot(data = total_slr_demand_growth, 
                                      mapping = aes(x = time,
                                                    y = `Policy Demand: Erosion Control`,
                                                    color = Scenario)) +
  geom_line(size = 6,
...
total_crabs_growth_demand_plot
...
ggsave(filename = "figures/erosion_growth_demand_plots.png",
       width = 35,
       height = 30)

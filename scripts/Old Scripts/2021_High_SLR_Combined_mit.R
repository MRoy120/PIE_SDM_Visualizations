...
high_SLR_combined_mit <- read_csv("data/scenario_5_combined_mitigation/2021_High_SLR_Combined_Mit.csv") %>%
...
ggplot(data = high_SLR_combined_mit_2, 
       mapping = aes(x = time,
                     y = areal_extent,
                     color = `Habitat Type`)) +
  geom_line(size = 6,
  ...
  ggsave(filename = "figures/high_SLR_combined_mit.png",
         width = 22,
         height = 15)

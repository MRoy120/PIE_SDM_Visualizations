...
low_SLR_base <- read_csv("data/2021_Baseline_Low_SLR.csv") %>%
...
ggplot(data = low_SLR_base_2, mapping = aes(x = time,
  ...
  labs(title = "Plum Island Estuary Salt Marsh Extent",
       subtitle = "Marsh Area by Habitat Type at\nLow Sea Level Rise Without Crabs or Mitigation Strategies") +
  theme(plot.title = element_text(size = 42, 
                                  face = "bold"),
        plot.subtitle = element_text(size = 38,
                                     face = "bold"),
        legend.position = "bottom") +
  ggsave(filename = "scripts/simulation_visualization/figures/low_SLR_baseline.png",
         width = 22,
         height = 15)

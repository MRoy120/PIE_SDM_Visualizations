...
low_SLR_combined_mit <- read_csv("data/2021_Low_SLR_Combined_Mit.csv") %>%
...
ggplot(data = low_SLR_combined_mit_2, mapping = aes(x = time,
                                             y = areal_extent,
                                             color = `Habitat Type`)) +
  geom_line(size = 6,
  ...
  labs(title = "Plum Island Estuary Salt Marsh Extent",
       subtitle = "Marsh Area by Habitat Type at\nLow Sea Level Rise With Crabs, with Combined Mitigation Policies") +
  theme(plot.title = element_text(size = 42, 
                                  face = "bold"),
        plot.subtitle = element_text(size = 38,
                                     face = "bold"),
        legend.position = "bottom") +
  ggsave(filename = "scripts/simulation_visualization/figures/low_SLR_combined_mit.png",
         width = 22,
         height = 15)

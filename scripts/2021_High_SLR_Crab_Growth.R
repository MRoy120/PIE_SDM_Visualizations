...
high_SLR_crab_growth <- read_csv("data/2021_High_SLR_Crab_Growth.csv") %>%
...
ggplot(data = high_SLR_crab_growth_2, mapping = aes(x = time,
                                                    y = areal_extent,
                                                    color = `Habitat Type`)) +
  geom_line(size = 6,
  ...
  labs(title = "Plum Island Estuary Salt Marsh Extent",
       subtitle = "Marsh Area by Habitat Type at\nHigh Sea Level Rise With Crab Marsh Growth") +
  theme(plot.title = element_text(size = 42, 
                                  face = "bold"),
        plot.subtitle = element_text(size = 38,
                                     face = "bold"),
        legend.position = "bottom") +
  ggsave(filename = "scripts/simulation_visualization/figures/high_SLR_crab_growth.png",
         width = 22,
         height = 15)
